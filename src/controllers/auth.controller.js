...
const generateToken = (id) =>
    jwt.sign({ id }, "gi7gug9ug9o88iohoyyyyyyy89yuyuuyuuuuuy8676rrr6rr", {
...
const register = async (req, res) => {
    try {
        const { companyName, address, companyEmail, password, userName, pincode, state, country, channelName } = req.body;
        ...
        const company = await Company.create({
            name: companyName.trim(), address: address || "",
            email: companyEmail.toLowerCase(),
            passwordHash: await bcrypt.hash(password, 10),
        });

        const user = await User.create({
            companyId: company._id, name: userName.trim(),
            email: companyEmail.toLowerCase(),
            passwordHash: await bcrypt.hash(password, 10),
            role: "admin",
        ...
        const channel = await Channel.create({
            companyId: company._id, name: channelName.trim(),
            createdBy: user._id, members: [{ userId: user._id, canPost: true }],
        });
        ...
        await Session.create({
            ...
            deviceInfo: req.headers["user-agent"] || null, ipAddress: req.ip,
        });

        sendTokenResponse(res, token, 201, {
            user: { id: user._id, name: user.name, email: user.email, role: user.role, companyId: company._id, avatar_url: user.avatar_url, theme: user.theme },
    ...
    try {
        ...
        sendTokenResponse(res, token, 200, {
            user: {
                ...
                companyId: user.companyId, avatar_url: user.avatar_url, designation: user.designation,
...
const forgotPassword = async (req, res) => {
    try {
        ...
        await PasswordReset.create({
            userId: user._id, tokenHash: hashToken(rawToken),
            expiresAt: new Date(Date.now() + 60 * 60 * 1000),
        });
        console.log(`[DEV] Reset token for ${user.email}: ${rawToken}`);
        // Return devToken only in development — remove in production
        res.json({ message: "If that email exists, a reset link has been sent", devToken: process.env.NODE_ENV !== "production" ? rawToken : undefined });
    } catch (error) {
        res.status(500).json({ message: "Server error", error: error.message });
    }
};
...
const resetPassword = async (req, res) => {
    try {
        ...
        const record = await PasswordReset.findOne({ tokenHash: hashToken(token), usedAt: null, expiresAt: { $gt: new Date() } });
        if (!record) return res.status(400).json({ message: "Invalid or expired reset token" });
        await User.findByIdAndUpdate(record.userId, { passwordHash: await bcrypt.hash(newPassword, 10) });
...
const changePassword = async (req, res) => {
    try {
        ...
        await Session.deleteMany({ userId: user._id, tokenHash: { $ne: hashToken(currentToken) } });
...
const getSessions = async (req, res) => {
    try {
        const sessions = await Session.find({ userId: req.user._id }).select("-tokenHash").sort({ createdAt: -1 });
        res.json(sessions);
    } catch (error) {
        res.status(500).json({ message: "Server error", error: error.message });
    }
